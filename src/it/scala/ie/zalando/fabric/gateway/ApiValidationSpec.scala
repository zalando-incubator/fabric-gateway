  ...
  it should "Create routes for handling OPTIONS requests when CORS is enabled" in {
    synchRequest(ValidSynchRequestWithCorsEnabled.payload) ~> Route.seal(createRoutesFromDerivations(ingressDerivationLogic)) ~> check {
      ...
      corsRoutes should contain theSameElementsAs List(
        """Path("/api/resource") && Method("OPTIONS") && Header("X-Forwarded-Proto", "https") -> enableAccessLog(4, 5) -> status(204) -> flowId("reuse") -> corsOrigin("https://example.com", "https://example-other.com") -> appendResponseHeader("Access-Control-Allow-Methods", "POST, OPTIONS") -> appendResponseHeader("Access-Control-Allow-Headers", "X-Flow-Id, Content-Type") -> appendResponseHeader("Access-Control-Allow-Credentials", "true") -> <shunt>""",
        """Path("/api/resource/:id") && Method("OPTIONS") && Header("X-Forwarded-Proto", "https") -> enableAccessLog(4, 5) -> status(204) -> flowId("reuse") -> corsOrigin("https://example.com", "https://example-other.com") -> appendResponseHeader("Access-Control-Allow-Methods", "GET, PATCH, PUT, OPTIONS") -> appendResponseHeader("Access-Control-Allow-Headers", "X-Flow-Id, Content-Type") -> appendResponseHeader("Access-Control-Allow-Credentials", "true") -> <shunt>""",
